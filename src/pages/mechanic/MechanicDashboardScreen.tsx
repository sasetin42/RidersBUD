    ...
    return (
        <div className="bg-dark-gray p-4 rounded-lg">
             <div className="flex justify-between items-center mb-3">
                ...
                <span className="text-xs text-light-gray">{activeTasks.length} pending</span>
            </div>
                ...
                {activeTasks.slice(0, 3).map(task => (
                    <div key={task.id} className="flex items-center gap-2 text-sm">
                        <input 
                            ...
                            className="rounded border-gray-500 bg-field text-primary focus:ring-primary"
                        />
                        <span className="text-white">{task.title}</span>
                    </div>
                ))}
                {activeTasks.length === 0 && <p className="text-xs text-light-gray italic">No pending tasks.</p>}
            </div>
            <form onSubmit={handleSubmit} className="flex gap-2">
                <input 
                    ...
                    className="flex-grow bg-field border border-secondary rounded px-2 py-1 text-sm text-white placeholder-light-gray focus:outline-none focus:ring-1 focus:ring-primary"
                />
                <button type="submit" className="bg-primary text-white px-3 py-1 rounded text-sm font-bold hover:bg-orange-600">+</button>
            </form>
        </div>
    );
};
...
const MechanicDashboardScreen: React.FC = () => {
    ...
    const { db, loading, acceptJobRequest, addTask, updateTask } = useDatabase();
                ...
                {analyticsData && (
                    <div className="space-y-6">
                        ...
                        <DashboardTasksWidget 
                            ...
                            onToggleTask={handleQuickToggleTask} 
                        />
