...
const SignUpScreen: React.FC = () => {
    ...
    const handleSignUp = (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        // In a real app, you'd register the user via an API.
        setTimeout(() => {
            console.log("Signing up with:", { name, email });
            login();
            // After login, the App component will redirect to the home screen.
        }, 1500);
    };
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-secondary p-8">
            {settings.authLogoUrl ? (
                ...
                <h1 className="text-5xl font-bold text-primary mb-4">{settings.signupTitle || 'RidersBUD'}</h1>
            )}
            <p className="text-light-gray mb-12">{settings.signupSubtitle || 'Create your account'}</p>
            ...
            <div className="w-full max-w-sm">
                 <form onSubmit={handleSignUp}>
                    ...
                    >
                        {isLoading ? <Spinner size="sm" color="text-white"/> : 'Sign Up'}
                    </button>
                </form>
